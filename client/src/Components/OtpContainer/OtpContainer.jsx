...
const OtpContainer = () => {
  ...
  const emailhandleSubmit = async (e) => {
    ...
    const res = await axios.post("http://localhost:5000/otp/sendotp", data);
  };
    ...
    <div className="container">
      <div className="chatvia-registeration">
        ...
        <div className="chatvia-register-header">
          {error && <div className="text-danger"> Invalid OTP/Email</div>}
          <div>Sign in with Otp</div>
          <div>Sign in to continue to Chatvia.</div>
        </div>
        <div className="chatvia-register-form p-5">
          <form onSubmit={handleSubmit}>
            ...
            <div className="otp-form otpform">
              {otp.map((_, index) => (
                <input
                  key={index}
                  type="text"
        ...
        <p className="register-signin">
          © 2024 Chatvia. Crafted with ❤️ by Themesbrand
        </p>
      </div>
    </div>
  );
